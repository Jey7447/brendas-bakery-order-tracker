...
export default function Dashboard() {
  ...
  const selectedDayOrders = useMemo(() => orders.filter((order) => order.deliveryDate === selectedCalendarDate).sort((a, b) => a.deliveryTime.localeCompare(b.deliveryTime)), [orders, selectedCalendarDate]);
  ...
  return (
    <div className="dashboard-shell">
      ...
      <aside className={`sidebar ${mobileNav ? "open" : ""}`}>
        ...
        <button type="button" className={`side-item ${calendarView ? "active" : ""}`} onClick={() => { const today = getToday(); const parsed = parseLocalDate(today); setCalendarMonth(new Date(parsed.getFullYear(), parsed.getMonth(), 1)); setSelectedCalendarDate(today); setCalendarView(true); setMobileNav(false); }}><CalendarDays size={17} /> Calendar</button>
        ...
        {!calendarView && <section className="orders-card">
          <div className="orders-head"><div><h2>{view === "today" ? "Today" : view === "upcoming" ? "Upcoming" : view === "unpaid" ? "Unpaid orders" : "All orders"}</h2><p>Sorted automatically by soonest delivery.</p></div><div className="order-tools" style={{ position: "relative" }}><div className="search"><Search size={16} /><input placeholder="Search orders..." value={query} onChange={(e) => setQuery(e.target.value)} /></div><button type="button" className={`filter-button ${activeFilterCount ? "active" : ""}`} aria-label="Open filters" aria-expanded={filtersOpen} onClick={() => setFiltersOpen((open) => !open)}><Filter size={16} />{activeFilterCount > 0 && <span style={{ position: "absolute", top: -5, right: -5, minWidth: 16, height: 16, borderRadius: 99, background: "var(--ink)", color: "white", fontSize: 9, display: "grid", placeItems: "center", fontWeight: 800 }}>{activeFilterCount}</span>}</button>
            {filtersOpen && <div style={{ position: "absolute", top: 46, right: 0, zIndex: 20, width: 280, background: "var(--paper)", border: "1px solid var(--line)", borderRadius: 10, padding: 16, boxShadow: "0 18px 45px rgba(0,0,0,.14)" }} onClick={(e) => e.stopPropagation()}>
              ...
              <label style={{ display: "grid", gap: 6, fontSize: 10, fontWeight: 800, marginBottom: 12 }}>ORDER STATUS<select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value as OrderStatus | "ALL")} style={fieldStyle}><option value="ALL">All statuses</option>{Object.keys(statusLabel).map((s) => <option key={s} value={s}>{statusLabel[s as OrderStatus]}</option>)}</select></label>
              <label style={{ display: "grid", gap: 6, fontSize: 10, fontWeight: 800 }}>DELIVERY<select value={deliveryFilter} onChange={(e) => setDeliveryFilter(e.target.value as DeliveryFilter)} style={fieldStyle}><option value="ALL">All dates</option><option value="TODAY">Today</option><option value="UPCOMING">Upcoming</option></select></label>
              <button type="button" className="button light small" style={{ width: "100%", marginTop: 14 }} onClick={() => { resetFilters(); setFiltersOpen(false); }}>Clear filters</button>
            </div>}
          </div></div>
          <div className="view-tabs">{(["today", "upcoming", "all", "unpaid"] as const).map((v) => <button type="button" key={v} className={view === v ? "active" : ""} onClick={() => setView(v)}>{v === "today" ? "Today" : v === "upcoming" ? "Upcoming" : v === "unpaid" ? "Unpaid" : "All orders"}{v === "unpaid" && todayUnpaid > 0 && <span>{todayUnpaid}</span>}</button>)}</div>
          <div className="order-list">
            {sorted.length > 0 ? sorted.map((order, index) => <div key={`${order.id}-${order.rowNumber ?? index}`} className={`order-row ${order.paymentStatus === "UNPAID" && order.deliveryDate === TODAY ? "needs-payment" : ""}`}><div className="order-time"><strong>{order.deliveryTime}</strong><span>{order.deliveryDate === TODAY ? "Today" : order.deliveryDate}</span></div><div className="order-customer"><strong>{order.customerName}</strong><span>{order.phone}</span></div><div className="order-status"><span className={`status-dot ${order.orderStatus.toLowerCase().replaceAll("_", "-")}`} /><select aria-label={`Status for ${order.id}`} value={order.orderStatus} onChange={(e) => updateOrderStatus(order.id, e.target.value as OrderStatus)} disabled={statusSavingId === order.id}><option value="PENDING">Pending</option><option value="CONFIRMED">Confirmed</option><option value="PREPARING">Preparing</option><option value="READY">Ready</option><option value="OUT_FOR_DELIVERY">Out for delivery</option><option value="DELIVERED">Delivered</option><option value="CANCELLED">Cancelled</option></select></div><div className={`payment-pill ${order.paymentStatus.toLowerCase()}`}>{order.paymentStatus === "PAID" ? <Check size={13} /> : <CircleDollarSign size={13} />}{order.paymentStatus === "PAID" ? "Paid" : "Unpaid"}</div><div className="order-total">{money(order.total)}</div><div>{order.paymentStatus === "UNPAID" && <button type="button" className="pay-button" disabled={payingId === order.id} onClick={() => markPaid(order.id)}>{payingId === order.id ? "…" : "Mark paid"}</button>}</div><button type="button" className="row-more" aria-label={`View ${order.id}`} onClick={() => setViewing(order)}><ChevronRight size={17} /></button></div>) : <div className="no-orders"><CircleDollarSign size={28} /><span>{loading ? "Loading orders…" : "No orders match this view."}</span></div>}
          </div>
        </section>}
      </main>

      {viewing && <div className="overlay" onClick={() => !payingId && setViewing(null)}><aside style={panelStyle} onClick={(e) => e.stopPropagation()}><div className="cart-head"><div><div className="eyebrow">ORDER {viewing.id}</div><h2>{viewing.customerName}</h2></div><button type="button" disabled={Boolean(payingId)} onClick={() => setViewing(null)}><X /></button></div>
        <div style={{ padding: "25px 0", display: "grid", gap: 22 }}>
          <div><strong style={{ fontSize: 15 }}>{viewing.customerName}</strong><span style={{ display: "block", fontSize: 12, color: "var(--muted)", marginTop: 5 }}>{viewing.phone}</span><span style={{ display: "block", fontSize: 12, color: "var(--muted)", marginTop: 3 }}>{viewing.email}</span></div>
          <div><div className="eyebrow">ITEMS</div>{viewing.items.map((item, index) => <div key={`${item.name}-${index}`} style={{ display: "flex", justifyContent: "space-between", gap: 15, padding: "12px 0", borderBottom: "1px solid var(--line)", fontSize: 12 }}><span>{item.name} × {item.quantity}</span>{item.unitPrice > 0 && <strong>{money(item.unitPrice * item.quantity)}</strong>}</div>)}<div style={{ display: "flex", justifyContent: "space-between", paddingTop: 16 }}><span style={{ fontSize: 12, fontWeight: 700 }}>Total</span><strong style={{ fontFamily: "DM Serif Display", fontSize: 24 }}>{money(viewing.total)}</strong></div></div>
          <div><div className="eyebrow">DELIVERY</div><strong style={{ display: "block", fontSize: 13, marginTop: 5 }}>{viewing.deliveryDate === TODAY ? "Today" : viewing.deliveryDate}</strong><span style={{ display: "block", fontSize: 12, color: "var(--muted)", marginTop: 4 }}>{viewing.deliveryTime}</span><span style={{ display: "block", fontSize: 12, lineHeight: 1.6, marginTop: 8 }}>{viewing.address}</span></div>
          <div><div className="eyebrow">PAYMENT</div><div className={`payment-pill ${viewing.paymentStatus.toLowerCase()}`} style={{ width: "fit-content", marginTop: 6 }}>{viewing.paymentStatus === "PAID" ? <Check size={13} /> : <CircleDollarSign size={13} />}{viewing.paymentStatus === "PAID" ? "Paid" : "Unpaid"}</div></div>
          <div><div className="eyebrow">ORDER STATUS</div><div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 6 }}><span className={`status-dot ${viewing.orderStatus.toLowerCase().replaceAll("_", "-")}`} /><span style={{ fontSize: 12, fontWeight: 700 }}>{statusLabel[viewing.orderStatus]}</span></div></div>
          {viewing.notes && <div><div className="eyebrow">NOTES</div><p style={{ margin: "6px 0 0", fontSize: 12, lineHeight: 1.6 }}>{viewing.notes}</p></div>}
        </div>
        <div className="edit-footer">{viewing.paymentStatus === "UNPAID" && <button type="button" className="button light" disabled={payingId === viewing.id} onClick={() => markPaid(viewing.id)}>{payingId === viewing.id ? "Saving…" : <>Mark paid <Check size={16} /></>}</button>}<button type="button" className="button dark" disabled={Boolean(payingId)} onClick={() => { setEditing(viewing); setViewing(null); }}>Edit order <ChevronRight size={16} /></button></div>
      </aside></div>}

      {editing && <div className="overlay" onClick={() => !savingId && setEditing(null)}><aside className="edit-panel" onClick={(e) => e.stopPropagation()}><div className="cart-head"><div><div className="eyebrow">ORDER {editing.id}</div><h2>{editing.customerName}</h2></div><button type="button" disabled={Boolean(savingId)} onClick={() => setEditing(null)}><X /></button></div><div className="edit-body">
      ...
      </div><div className="edit-footer"><button type="button" className="button light" disabled={Boolean(savingId)} onClick={() => setEditing(null)}>Cancel</button><button type="button" className="button dark" disabled={savingId === editing.id} onClick={saveOrderChanges}>{savingId === editing.id ? "Saving…" : <>Save changes <Check size={16} /></>}</button></div></aside></div>}

      {adding && <div className="overlay" onClick={() => !addingOrder && setAdding(false)}><aside style={panelStyle} onClick={(e) => e.stopPropagation()}><div className="cart-head"><div><div className="eyebrow">NEW ORDER</div><h2>Add an order</h2></div><button type="button" disabled={addingOrder} onClick={() => setAdding(false)}><X /></button></div>
