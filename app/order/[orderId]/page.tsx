    ...
    async function load(showLoading = false) {
      ...
      try {
        ...
        if (!response.ok || result?.ok === false) {
          throw new Error(result?.message || "We couldn't load your order.");
        }
        ...
        if (!active) return;
        if (showLoading) {
          setError(err instanceof Error ? err.message : "We couldn't load your order details right now.");
        }
      } finally {
    ...
    return (
      <main className="order-page">
        <div className="order-loading">
          <div className="order-check"><Clock3 size={26} /></div>
          <h1>Getting your order ready…</h1>
          <p>Just a moment while we fetch the latest details.</p>
        </div>
      </main>
    );
  }
    ...
    return (
      <main className="order-page">
        <div className="order-error">
          ...
          <h1>Something went a little wrong.</h1>
          <p>{error || "Order not found."}</p>
          <Link href="/" className="button dark">
            Back to the bakery <ArrowRight size={17} />
          </Link>
        </div>
      </main>
    );
    ...
    <main className="order-page">
      <div className="order-wrap">
        <Link href="/" className="order-back">
          <ArrowLeft size={15} /> Brenda&apos;s Bakery
        </Link>
        ...
        <section className="order-hero-card">
          ...
          <h1>
            Yay, {order.customerName?.split(" ")[0] || "there"}!<br />
            <em>Your order is in.</em>
          </h1>
          <p>
            We&apos;ve received your order and Brenda&apos;s Bakery is getting everything ready for you.
          </p>
        ...
        <section className="order-progress-card">
          <div className="order-card-heading">
            <div>
              <div className="eyebrow">YOUR ORDER JOURNEY</div>
              <h2>We&apos;ll keep things moving.</h2>
            </div>
            <div className="order-status-meta">
              <span className="live-indicator"><i className={live ? "live-dot" : ""} /> Live updates</span>
              <span className="status-pill">{titleStatus(order.orderStatus)}</span>
            </div>
          </div>
          ...
          <div className="order-progress">
            {steps.map((step, index) => {
              ...
              return (
                <div
                  className={`progress-step ${done ? "done" : ""} ${index === currentIndex ? "current" : ""}`}
                  key={step.key}
                >
                  <div className="progress-icon"><Icon size={15} /></div>
        ...
        <div className="order-content-grid">
            ...
            <div className="order-items">
              {itemLines.length ? (
                itemLines.map((item, index) => {
                  ...
                  return (
                    <div className="order-item" key={`${item.productId || item.name}-${index}`}>
                      <div className="order-item-image">
                        {product ? <img src={product.image} alt="" /> : <ShoppingBag size={18} />}
                      </div>
                      <div>
                        <strong>{item.name || product?.name || "Bakery item"}</strong>
                        <span>Qty {quantity} · {money(unitPrice)} each</span>
                      </div>
                ...
                <p className="order-muted">
                  Your order has been received. Item details will appear once the order is synced.
                </p>
              )}
            </div>
          ...
          <aside className="order-detail-card delivery-card">
            ...
            <div className="payment-note">
              <span className={order.paymentStatus === "PAID" ? "paid-dot" : "unpaid-dot"} />
              <div>
                <strong>
                  {order.paymentStatus === "PAID" ? "Payment confirmed" : "Payment to be confirmed"}
                </strong>
                <small>
                  {order.paymentStatus === "PAID"
                    ? "Thanks — your order is fully paid."
                    : "Brenda will confirm payment with you after receiving the order."}
                </small>
              </div>
            </div>
          </aside>
        ...
        <section className="next-step-card">
          <div>
            ...
            <p>
              Keep this order number handy if you need to ask about your order. You can also return to the menu and discover something sweet for next time.
            </p>
          </div>
          <div className="next-step-actions">
            <Link href="/order/track" className="button light">
              Track your order <ArrowRight size={17} />
            </Link>
            <Link href="/#menu" className="button dark">
              Back to the menu <ArrowRight size={17} />
            </Link>
          </div>
        </section>
