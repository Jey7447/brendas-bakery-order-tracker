...
export default function OrderConfirmation() {
    ...
    async function load() {
      try {
        const response = await fetch(
          `/api/dashboard/orders?orderId=${encodeURIComponent(orderId)}`,
          { cache: "no-store" }
        );
        ...
        const orders: RawOrder[] = Array.isArray(result)
          ? result
        ...
        const found = orders.find(
          item => item.orderId === orderId || item["Order ID"] === orderId
        );

        if (found) {
          setOrder({
            orderId: String(found.orderId || found["Order ID"] || orderId),
            customerName: String(found.customerName || found["Customer Name"] || ""),
            items: found.items || (typeof found["Items"] === "string" ? [] : (found["Items"] as OrderItem[] | undefined)),
            total: Number(found.total ?? found["Total"] ?? 0),
            deliveryDate: String(found.deliveryDate || found["Delivery Date"] || ""),
            deliveryTime: String(found.deliveryTime || found["Delivery Time"] || ""),
            address: String(found.address || found["Address"] || ""),
            paymentStatus: String(found.paymentStatus || found["Payment Status"] || "UNPAID"),
            orderStatus: String(found.orderStatus || found["Order Status"] || "PENDING")
          });
        } else {
          setError("We couldn't find that order yet.");
        }
      } catch {
  ...
  const currentIndex = useMemo(
    () => Math.max(0, steps.findIndex(step => step.key === order?.orderStatus)),
    [order?.orderStatus]
  );
