...
import { ArrowLeft, ArrowRight, Check, Clock3, PackageCheck, RefreshCw, Search, ShoppingBag, Truck } from "lucide-react";
...
export default function TrackOrderPage() {
  ...
  async function fetchOrder(value: string, silent = false) {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    await fetchOrder(value);
  }

  useEffect(() => {
    if (!order?.orderId || order.orderStatus === "DELIVERED") return;

    const interval = window.setInterval(() => {
      void fetchOrder(order.orderId!, true);
    }, 15000);
        ...
        {order && (
          <section className="track-result" aria-live="polite">
            <div className="track-result-head">
              ...
              <div className="track-status-area">
                <span className="track-status">{(order.orderStatus || "PENDING").replaceAll("_", " ")}</span>
                <span className="track-live"><span className="track-live-dot" /> {refreshing ? "Updating…" : lastUpdated ? "Live updates on" : "Live updates"}</span>
              </div>
            </div>
